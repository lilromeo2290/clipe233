...
// ─── Main Component (Auth Wrapper) ─────────────────────────────────────────────

export default function AdminDashboard() {
  // Auth state only — no dashboard hooks here
  ...
  // Conditionally render login or dashboard — hooks are never called conditionally
  if (!isAuthenticated) {
    return <AdminLogin onLogin={handleLogin} />;
  }

  return <AdminDashboardInner authUser={authUser} onLogout={handleLogout} />;
}

// ─── Dashboard Inner Component ──────────────────────────────────────────────────
// All dashboard hooks live here so they are ALWAYS called in the same order.

function AdminDashboardInner({ authUser, onLogout }: { authUser: { username: string; role: string } | null; onLogout: () => void }) {
